...
CONSTRAINTS_FILE = DATA_FOLDER + os.sep +"constraints"
...
class ClingoApp(Application):
    ...
    def register_options(self, options):
        ...
        options.add(group, "holidays", _textwrap.dedent("""Holidays separated by ,"""), self.__parse_holiday)

    def __parse_holiday(self, option):        
        self.holidays = [int(h) for h in option.split(",")]

        if len(self.holidays) == 1 and self.holidays[0] == 0:
            self.holidays = []
            self.holidays_ask_input = False
            return True
            ...
            for constraint in _f.readlines():
                ...
                try:
                    if constraint[0] == "single":
                        ...
                        constraints += f"{constraint[0]}({constraint[1]}, ({','.join(constraint[2:])})).\n"
                except IndexError:
    ...
    def main(self, ctl, files):
        ...
        if self.holidays_ask_input and len(self.holidays) == 0:
